package com.sulake.habbo.widget.memenu
{
   ...
   public class EffectView
   {
      ...
      private function onUpdate(param1:Event = null) : void
      {
         ...
         {
            if(this.var_176.isActive)
            {
               _loc2_ = this.var_176.secondsLeft / Number(this.var_176.duration);
               this.var_889.width = _loc2_ * this.var_299;
            }
            else
            {
      ...
      private function setTimeLeft() : void
      {
         ...
         {
            this.var_890.caption = "${widgets.memenu.effects.active.timeleft}";
            _loc1_ = this.var_890.text;
            if(this.var_176.isActive)
            {
               _loc2_ = this.var_176.secondsLeft;
               ...
               this.var_890.text = _loc1_;
            }
            else
            {
               this.var_890.caption = "${widgets.memenu.effects.activate}";
            }
         }
      }
      ...
      private function createWindow(param1:String) : void
      {
         ...
         var _loc7_:* = null;
         var _loc3_:String = "";
         if(this.var_176.isInUse)
         {
            _loc3_ = "memenu_effect_selected";
         }
         else if(this.var_176.isActive)
         {
         ...
         else
         {
            ...
            this.var_888 = this._window.findChildByName("effect_hilite");
            this.var_888.visible = false;
         }
         this.var_890 = this._window.findChildByName("time_left") as ITextWindow;
         this.setTimeLeft();
         this.var_889 = this._window.findChildByName("loader_bar");
         if(this.var_889 != null)
         {
            this.var_299 = this.var_889.width;
            this.var_190.start();
            this.onUpdate();
         }
         if(this.var_176.icon)
         {
            this.setElementBitmap("effect_icon",this.var_176.icon);
         }
      }
