package com.sulake.habbo.widget.roomchat
{
   ...
   public class RoomChatItem
   {
      ...
      private static const const_592:int = 6;
      
      private static const const_1287:int = 6;
      
      private static const const_926:int = 35;
      ...
      public function RoomChatItem(param1:RoomChatWidget, param2:IHabboWindowManager, param3:IAssetLibrary, param4:String, param5:IHabboLocalizationManager, param6:String)
      {
         ...
         if(!var_213)
         {
            ...
            var_213.mouseThreshold = 0;
            var_213.setParamFlag(WindowParam.WINDOW_PARAM_FORCE_CLIPPING,true);
         }
         if(!var_404)
         {
            var_404 = this._windowManager.buildFromXML(this._assetLibrary.getAssetByName("bubble_shout").content as XML,1) as IRegionWindow;
         ...
         if(!var_405)
         {
            var_405 = this._windowManager.buildFromXML(this._assetLibrary.getAssetByName("bubble_whisper").content as XML,1) as IRegionWindow;
      ...
      {
         this.var_1354 = param1.chatType;
         this.var_1108 = param1.userId;
         this.var_893 = param1.userName;
         this.var_1107 = param1.userCategory;
         this._message = param1.text;
         this.var_488 = param1.links;
         this.var_1725 = param1.userX;
         this.var_245 = param1.userImage;
         this.var_2379 = param1.userColor;
      ...
      public function set y(param1:Number) : void
      {
         ...
         if(this._window != null)
         {
            this._window.y = param1 - this.var_725;
         }
      }
      ...
      {
         var _loc5_:Rectangle = new Rectangle(this._x,this.var_160,this.width,const_925);
      ...
      public function renderView() : void
      {
         ...
         if(this.var_245 && !isRespect)
         {
            ...
            if(userIconWindow)
            {
               ...
               userIconWindow.x = x1;
               userIconWindow.y = y1;
               this.var_725 = Math.max(0,(this.var_245.height - background.height) / 2);
         ...
         if(nameWindow != null)
         {
            if(!isRespect)
            {
               nameWindow.text = this.var_893 + ": ";
               nameWindow.y += this.var_725;
               nameWindow.width = nameWindow.textWidth + const_592;
            }
            else
            {
               nameWindow.text = "";
         ...
         if(this.var_1354 == RoomWidgetChatUpdateEvent.const_282)
         {
            textWindow.text = this.var_78.registerParameter("widgets.chatbubble.respect","username",this.var_893);
            this._width = const_926;
         }
         else if(this.var_1354 == RoomWidgetChatUpdateEvent.const_274)
         {
            textWindow.text = this.var_78.registerParameter("widget.chatbubble.petrespect","petname",this.var_893);
            this._width = const_926;
         }
         ...
         else
         {
            ...
            lastLinkEndPos = -1;
            i = 0;
            while(i < this.var_488.length)
            {
               filteredLink = this.var_488[i][1];
               placeHolder = "{" + i + "}";
               placeholderPos = this._message.indexOf(placeHolder);
               lastLinkEndPos = placeholderPos + filteredLink.length;
               this.var_724.push([placeholderPos,lastLinkEndPos]);
            ...
            i = 0;
            while(i < this.var_724.length)
            {
               linkPos = this.var_724[i];
               try
               {
         ...
         if(textWindow.visible)
         {
            ...
            if(nameWindow != null)
            {
               ...
               if(nameWindow.width > const_592)
               {
                  textWindow.x -= const_592 - 1;
               }
            }
            textWindow.y += this.var_725;
            messageWidth = textWindow.textWidth;
            textWindow.width = messageWidth + const_1287;
         ...
         if(pointerWindow != null && pointerWindow.visible)
         {
            ...
            pointerWindow.y += this.var_725;
         }
         var bitmap:BitmapData = this.buildBubbleImage(nameWindow.width + textWindow.width,background.height,this.var_2379,isRespect);
         this._window.width = bitmap.width;
         this._window.y -= this.var_725;
         this._width = this._window.width;
         background.bitmap = bitmap;
         background.y = this.var_725;
      }
      ...
      private function onMessageLinkMouseClick(param1:WindowMouseEvent) : void
      {
         ...
         var _loc3_:int = 0;
         while(_loc3_ < this.var_724.length)
         {
            if(_loc2_ >= this.var_724[_loc3_][0] && _loc2_ <= this.var_724[_loc3_][1])
            {
               if(this.var_488[_loc3_][2] == 0)
               {
                  HabboWebTools.openExternalLinkWarning(this.var_488[_loc3_][0]);
               }
               else if(this.var_488[_loc3_][2] == 1)
               {
                  HabboWebTools.openWebPage(this.var_1726 + this.var_488[_loc3_][0],"habboMain");
               }
               else
               {
                  HabboWebTools.openWebPage(this.var_1726 + this.var_488[_loc3_][0]);
               }
            }
            _loc3_++;
         }
      ...
      {
         this._widget.onItemMouseClick(this.var_1108,this.var_893,this.var_1107,this._roomId,this._roomCategory,param1);
      }
      
      private function onBubbleMouseDown(param1:WindowMouseEvent) : void
      {
         this._widget.onItemMouseDown(this.var_1108,this.var_1107,this._roomId,this._roomCategory,param1);
      }
      
      private function onBubbleMouseOver(param1:WindowMouseEvent) : void
      {
         this._widget.onItemMouseOver(this.var_1108,this.var_1107,this._roomId,this._roomCategory,param1);
      }
      
      private function onBubbleMouseOut(param1:WindowMouseEvent) : void
      {
         this._widget.onItemMouseOut(this.var_1108,this.var_1107,this._roomId,this._roomCategory,param1);
      }
