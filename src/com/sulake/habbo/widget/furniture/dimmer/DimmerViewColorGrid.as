package com.sulake.habbo.widget.furniture.dimmer
{
   ...
   public class DimmerViewColorGrid
   {
      ...
      private var var_1188:BitmapData;
      ...
      public function setSelectedColorIndex(param1:int) : void
      {
         ...
         if(param1 < 0 || param1 >= this.var_519.numGridItems)
         {
            return;
         }
         this.select(this.var_519.getGridItemAt(param1) as IWindowContainer);
      }
      
      private function populate(param1:IHabboWindowManager) : void
      {
         if(this._view == null || this.var_519 == null)
         {
            return;
         }
      ...
      private function populateColourGrid(param1:IHabboWindowManager) : void
      {
         ...
         {
            _loc3_ = param1.buildFromXML(this.var_1893) as IWindowContainer;
            ...
            _loc3_.width = this.var_634.width;
            _loc3_.height = this.var_634.height;
            this.var_519.addGridItem(_loc3_);
            _loc4_ = _loc3_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
            if(_loc4_ != null)
            {
               _loc4_.bitmap = new BitmapData(this.var_634.width,this.var_634.height,true,0);
               _loc4_.bitmap.copyPixels(this.var_634,this.var_634.rect,new Point(0,0));
            }
            _loc5_ = _loc3_.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
            if(_loc5_ != null)
            {
               _loc5_.bitmap = new BitmapData(this.var_1188.width,this.var_1188.height,true,0);
      ...
      {
         var _loc2_:int = this.var_519.getGridItemIndex(param1.target as IWindow);
