package com.sulake.habbo.communication.demo
{
   ...
   public class SSOTicket extends EventDispatcher
   {
      ...
      public static const const_298:String = "failure";
      ...
      private var var_1439:IHabboWebLogin;
      ...
      public function SSOTicket(param1:IAssetLibrary, param2:IHabboWebLogin, param3:String, param4:Boolean = true)
      {
         super();
         this._assets = param1;
         this.var_1439 = param2;
         this.var_1440 = "http://" + param3 + "/client";
         if(!param4)
         {
            this.var_1439.init();
            this.var_1439.addEventListener(HabboWeb.const_209,this.requestClientURL);
         }
         else
         {
      ...
      {
         var _loc2_:String = this.var_1440;
         if(this._assets.hasAsset(_loc2_))
         {
            Logger.log("[CoreLocalizationManager] reload localization for url: " + this.var_1440);
         }
         var _loc3_:URLRequest = new URLRequest(this.var_1440);
         if(this._assets.hasAsset(_loc2_))
         {
      ...
      private function clientURLComplete(param1:Event = null) : void
      {
         ...
         {
            this.var_1439.requestReAuthenticate();
         }
         else
         {
            try
            {
               ...
               this.var_2636 = /\"flash.client.url\" : \"(.*?)\"/.exec(data)[1];
               facebookData = /\"facebook.user\" : \"(.*?)\"/.exec(data);
               if(facebookData)
               {
                  this.var_2637 = Boolean(facebookData[1]);
               }
