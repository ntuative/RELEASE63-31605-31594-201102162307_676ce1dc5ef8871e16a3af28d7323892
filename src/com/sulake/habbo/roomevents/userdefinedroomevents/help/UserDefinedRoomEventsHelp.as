package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private var var_1154:IContainerButtonWindow;
      ...
      private function prepareWindow() : void
      {
         ...
         this.var_1823 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1154 = this.var_1823.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1154)
         {
            this.var_1154.procedure = this.onButtonClick;
         }
         this.var_1155 = this.var_1823.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1155)
         {
            this.var_1155.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_258 >= this.var_78.length - 1)
         {
            this.var_1155.disable();
         }
         else
         {
            this.var_1155.enable();
         }
         if(this.var_258 <= 0)
         {
            this.var_1154.disable();
         }
         else
         {
            this.var_1154.enable();
         }
      }
      ...
      {
         ++this.var_258;
         this.var_258 = Math.min(this.var_258,this.var_78.length - 1);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         while(this.var_89.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            ...
            this.var_78.push([_loc2_,_loc3_,_loc4_]);
            _loc1_++;
         }
      }
      ...
      private function localize() : void
      {
         ...
         if(!this.var_78 || this.var_78.length == 0)
         {
            return;
         }
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_78[this.var_258][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_78[this.var_258][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_78[this.var_258][1]).value;
