package com.sulake.habbo.advertisement
{
   ...
   public class AdManager extends Component implements IAdManager
   {
      ...
      private var var_1057:BitmapData = null;
      ...
      private function onConfigurationReady(param1:IID = null, param2:IUnknown = null) : void
      {
         ...
         this.var_2233 = this._config.getKey("interstitial.enabled","false") == "true";
         this.var_2234 = int(this._config.getKey("interstitial.interval","30000"));
         this.var_2235 = int(this._config.getKey("interstitial.show.time","5000"));
         this.var_1630 = int(this._config.getKey("interstitial.max.displays.flash","-1"));
      ...
      public function showInterstitial() : Boolean
      {
         ...
         if(this.var_210 != null && this.var_210.running)
         {
            this.var_210.reset();
            return false;
         }
         if(this.var_1056 == 0)
         {
            this.var_1056 = getTimer();
            return false;
         }
         if(this.var_1630 > 0 && this.var_1631 >= this.var_1630)
         {
            return false;
         }
         var _loc1_:int = getTimer() - this.var_1056;
         if(_loc1_ > this.var_2234)
         {
            ++this.var_1631;
      ...
      private function onInterstitial(param1:InterstitialMessageEvent) : void
      {
         ...
         this.var_1299 = _loc2_.imageUrl;
         this.var_2232 = _loc2_.clickUrl;
         this.var_1056 = getTimer();
         if(this.var_1299 != "")
         {
            _loc3_ = this.var_1299;
            ...
            {
               this.var_210 = new Timer(this.var_2235,1);
               this.var_210.addEventListener(TimerEvent.TIMER_COMPLETE,this.interstitialTimerComplete);
            }
            if(this.var_210.running)
            {
               this.var_210.reset();
            }
            this.var_210.start();
            _loc4_ = new URLRequest(this.var_1299);
      ...
      private function startInterstitial(param1:Bitmap) : void
      {
         ...
         {
            events.dispatchEvent(new AdEvent(AdEvent.const_442,0,0,param1.bitmapData,this.var_2232));
            this.var_1056 = getTimer();
         }
         else
         {
      ...
      private function stopInterstitial() : void
      {
         ...
         if(events != null)
         {
            events.dispatchEvent(new AdEvent(AdEvent.const_559));
         }
      }
      ...
      private function onRoomAd(param1:RoomAdMessageEvent) : void
      {
         ...
         this.var_1300 = _loc2_.imageUrl;
         this.var_1629 = _loc2_.clickUrl;
         if(this.var_1300 != "" && this.var_1629 != "")
         {
            _loc3_ = this.var_1300;
            if(assets.hasAsset(_loc3_))
            {
               assets.removeAsset(assets.getAssetByName(_loc3_));
            }
            _loc4_ = new URLRequest(this.var_1300);
            _loc5_ = assets.loadAssetFromFile(_loc3_,_loc4_,"image/jpeg");
      ...
      private function roomAdImageReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(this.isValidAdImage(_loc3_))
         {
            ...
            events.dispatchEvent(new AdEvent(AdEvent.const_557,0,0,_loc4_,this.var_1629,this.var_1057,this.var_1055));
         }
      }
      ...
      private function adWarningLeftReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(_loc3_ != null)
         {
            this.var_1057 = this.emulateBackgroundTransparency(_loc3_.bitmapData);
         }
      }
      ...
      private function adWarningRightReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(_loc3_ != null)
         {
            this.var_1055 = this.emulateBackgroundTransparency(_loc3_.bitmapData);
         }
      }
