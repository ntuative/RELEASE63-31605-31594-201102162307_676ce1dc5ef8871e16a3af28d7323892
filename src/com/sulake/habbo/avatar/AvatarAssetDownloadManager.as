package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_935:int = 3;
      
      private var var_414:AssetLoaderStruct;
      
      private var var_936:Timer;
      ...
      private const const_1975:int = 100;
      
      private const const_1562:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_936 = new Timer(this.const_1975,1);
         this.var_936.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_414)
         {
            this.var_414.addEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_414.addEventListener(AssetLoaderEvent.const_46,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_414)
         {
            this.var_414.removeEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_414.removeEventListener(AssetLoaderEvent.const_46,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_935);
            if(this.var_1419.indexOf("?") > 0)
            {
               _loc2_ = this.var_1419 + "&retry=" + this.var_935;
            }
            else
            {
               _loc2_ = this.var_1419 + "?retry=" + this.var_935;
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2539,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1160[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1160[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1828)
         {
            this.var_1161.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_446[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_446[_loc3_] = _loc6_;
            }
            this.var_934[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_934)
         {
            _loc7_ = true;
            _loc4_ = this.var_934[_loc3_];
            ...
            if(_loc7_)
            {
               ...
               delete this.var_446[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_934[_loc3_];
         }
         while(_loc6_ < this.var_753.length)
         {
            _loc5_ = this.var_753[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_753.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_936.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_754.length > 0 && this.var_753.length < this.const_1562)
         {
            _loc1_ = this.var_754[0];
            _loc1_.startDownloading();
            this.var_753.push(this.var_754.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_754.indexOf(param1) == -1 && this.var_753.indexOf(param1) == -1)
         {
            this.var_754.push(param1);
            this.processPending();
         }
      }
