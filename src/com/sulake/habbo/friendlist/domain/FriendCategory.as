package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategory implements IDisposable
   {
      ...
      {
         if(this.var_977 >= this.getPageCount())
         {
            this.var_977 = Math.max(0,this.getPageCount() - 1);
         }
      }
      
      public function getPageCount() : int
      {
         return Math.ceil(this.var_225.length / const_340);
      }
      
      public function getStartFriendIndex() : int
      {
         this.checkPageIndex();
         return this.var_977 * const_340;
      }
      
      public function getEndFriendIndex() : int
      {
         this.checkPageIndex();
         return Math.min((this.var_977 + 1) * const_340,this.var_225.length);
      }
