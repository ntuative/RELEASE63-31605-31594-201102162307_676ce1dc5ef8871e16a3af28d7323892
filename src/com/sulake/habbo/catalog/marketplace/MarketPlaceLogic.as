package com.sulake.habbo.catalog.marketplace
{
   ...
   public class MarketPlaceLogic implements IMarketPlace
   {
      ...
      public const const_1789:int = 1;
      
      public const const_1882:int = 2;
      
      public const const_1997:int = 3;
      ...
      private var var_2137:MarketplaceItemStats;
      ...
      public function dispose() : void
      {
         ...
         if(this.var_170)
         {
            for each(_loc1_ in this.var_170)
            {
               if(_loc1_ != null)
               {
                  _loc1_.dispose();
      ...
      {
         this.requestOffers(this.var_2136,this.var_2141,this.var_2140,this.var_2139);
      }
      ...
      {
         if(!this.var_170 || !this._catalog || !this._catalog.getPurse())
         {
            return;
         }
         var _loc2_:MarketPlaceOfferData = this.var_170.getValue(param1) as MarketPlaceOfferData;
         if(!_loc2_)
         {
            return;
         }
         ...
         this.showConfirmation(this.const_1789,_loc2_);
      }
      ...
      public function redeemSoldOffers() : void
      {
         var _loc2_:int = 0;
         var _loc3_:* = null;
         var _loc1_:Array = this.var_206.getKeys();
         for each(_loc2_ in _loc1_)
         {
            _loc3_ = this.var_206.getValue(_loc2_);
            if(_loc3_.status == MarketPlaceOfferState.const_421)
            {
               this.var_206.remove(_loc2_);
               _loc3_.dispose();
            }
         }
      ...
      public function onOffers(param1:IMessageEvent) : void
      {
         ...
         if(this.var_170)
         {
            for each(_loc5_ in this.var_170)
            {
               if(_loc5_ != null)
               {
                  _loc5_.dispose();
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            this.var_170.add(_loc4_.offerId,_loc6_);
         }
      ...
      public function onOwnOffers(param1:IMessageEvent) : void
      {
         ...
         if(this.var_206)
         {
            for each(_loc5_ in this.var_206)
            {
               if(_loc5_ != null)
               {
                  _loc5_.dispose();
         ...
         this.var_2138 = _loc3_.creditsWaiting;
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            _loc6_.timeLeftMinutes = _loc4_.timeLeftMinutes;
            this.var_206.add(_loc4_.offerId,_loc6_);
         }
         if(this._visualization == null)
         {
            return;
      ...
      public function onBuyResult(param1:IMessageEvent) : void
      {
         ...
         {
            item = this.var_170.remove(parser.requestedOfferId);
            if(item != null)
            {
               item.dispose();
            }
         ...
         {
            updateItem = this.var_170.getValue(parser.requestedOfferId) as MarketPlaceOfferData;
            if(updateItem)
            {
               updateItem.offerId = parser.offerId;
               updateItem.price = parser.newPrice;
               --updateItem.offerCount;
               this.var_170.add(parser.offerId,updateItem);
            }
            this.var_170.remove(parser.requestedOfferId);
            this.showConfirmation(this.const_1882,updateItem);
            this._visualization.listUpdatedNotify();
         }
      ...
      public function onCancelResult(param1:IMessageEvent) : void
      {
         ...
         if(parser.result == 1)
         {
            item = this.var_206.remove(parser.offerId);
            if(item != null)
            {
               item.dispose();
            }
      ...
      {
         if(param1.furniCategoryId != this.var_2105 || param1.furniTypeId != this.var_2110)
         {
            return;
         }
