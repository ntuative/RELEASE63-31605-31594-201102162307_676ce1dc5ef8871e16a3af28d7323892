package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_428 = this.getPetTypeIndexFromProduct(_loc3_.localizationId);
         if(this.var_428 >= 8)
         {
            return false;
         }
         this.updateAvailableBreeds(this.var_428);
         this.var_336 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc5_ = _loc4_.avatarRenderManager.petDataManager.getPetData(this.var_428,this);
               if(_loc5_)
               {
                  this.var_336 = _loc5_.colors;
         ...
         this._offers.add(this.var_428,_loc3_);
         events.addEventListener(WidgetEvent.CWE_SELECT_PRODUCT,this.onSelectProduct);
         events.addEventListener(WidgetEvent.CWE_COLOUR_INDEX,this.onColourIndex);
         events.addEventListener(WidgetEvent.const_816,this.onApproveNameResult);
      ...
      public function onDropMenuEvent(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowEvent.const_60)
         {
            ...
            if(this.var_195 == null || _loc3_ >= this.var_195.length)
            {
               return;
            }
            this.var_702 = _loc3_;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         if(this.var_476 < 0 || this.var_476 > this.var_336.length)
         {
            this.var_476 = 0;
         }
         this.updateImage();
      ...
      {
         if(param1.petType == this.var_428)
         {
            this.var_195 = param1.sellableBreeds;
            this.var_702 = 0;
      ...
      private function updateBreedSelections() : void
      {
         ...
         {
            _loc5_ = this.var_195[_loc3_] as int;
            _loc2_.push(_loc1_.getKey(this.getRaceLocalizationKey(this.var_428,_loc5_),this.getRaceLocalizationKey(this.var_428,_loc5_)));
            _loc3_++;
         }
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_195 == null || this.var_702 >= this.var_195.length)
         {
            return;
         }
         var _loc2_:* = 0;
         if(this.var_476 >= 0 && this.var_476 < this.var_336.length)
         {
            _loc7_ = this.var_336[this.var_476] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:int = this.var_195[this.var_702] as int;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_428,_loc3_,_loc2_,AvatarScaleType.const_55,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_317,AvatarAction.const_812);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_476 >= this.var_336.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_336[this.var_476] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:int = this.var_195[this.var_702] as int;
      ...
      private function updateAvailableBreeds(param1:int) : void
      {
         ...
         {
            this.var_195 = (page.viewer.catalog as HabboCatalog).getSellablePetBreeds(param1);
            this.var_702 = 0;
            if(this.var_195 != null)
            {
               this.updateBreedSelections();
